...
- **すべての部署フォルダに `inbox/` サブフォルダを必ず作成する**
  - 他部署からの通知（アラート/共有/依頼）受信箱として使う
  - inbox/ ファイルのフォーマットは SKILL.md 「inbox/ システム」セクション参照
- **部署フォルダのトップに `CLAUDE.md` を配置する**
  - 当該部署の役割・サブフォルダ構造・運用ルールを記載
- **部署のサブフォルダは下記の各部署テンプレに従う**
...
type: department
name: 秘書室
role: 窓口・相談役・タスク管理・ケース蓄積
---

# 秘書室

何でもお気軽にどうぞ。TODO管理、壁打ち、メモ、何でも承ります。
依頼は7ステップ判断フローで処理し、完了時にケース記録を残します。

## サブフォルダ
- `inbox/` - クイックキャプチャ。他部署からの通知受信箱
- `todos/` - 日次タスク管理
- `notes/` - 壁打ち・相談メモ・意思決定ログ
- `experience/` - ケース記録 + パターン抽出（自律成長の核）
```

### experience/ INDEX（secretary/experience/INDEX.md）

```markdown
---
type: index
description: ケース記録の一覧
---

# Experience INDEX

このフォルダにはケース記録（`case-NNN-<種別>-YYYY-MM-DD.md`）が蓄積されます。
同種ケースが3件以上溜まったら `patterns/<種別>.md` に統合知見が生成されます。
...
（3件以上のケースが溜まると `patterns/` 配下にまとめが生成されます）
```

### ケース記録（secretary/experience/case-NNN-template.md）
